...
######################
import src.ae_model as ae_model
...
encoder_input, encoder_output,  shape_before_flattening, encoder  = ae_model.build_encoder(input_dim = INPUT_DIM,
                                    output_dim = Z_DIM, 
                                    conv_filters = [32, 64, 64, 64],
                                    conv_kernel_size = [3,3,3,3],
                                    conv_strides = [2,2,2,2])

decoder_input, decoder_output, decoder = ae_model.build_decoder(input_dim = Z_DIM,
...
for file in input_list:
    print("changing class of "+file+" with autoencodeur ...")
    ...
    for vector in vector_list:
        ...
        if not os.path.exists('output/modified/autoencoder/{}'.format(file[0:-4])):
            os.mkdir('output/modified/autoencoder/{}'.format(file[0:-4]))
        matplotlib.image.imsave('output/modified/autoencoder/{}/{}.jpg'.format(file[0:-4],vector[0:-4]), img_decoded[0])  
